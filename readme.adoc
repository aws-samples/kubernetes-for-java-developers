...
== Custom JRE for Spring Boot Application

. Download and `scp` JDK 11 to an Amazon Linux instance
. Install JDK 11:
...
. Build Docker image using this custom JRE:
	...
	docker image build --file Dockerfile.jre -t arungupta/greeting:jre .

. List the Docker images and show the difference in sizes:

	[ec2-user@ip-172-31-21-7 app]$ docker image ls | grep greeting
	arungupta/greeting   jre                 c685bad6dd11        12 minutes ago      207MB
	arungupta/greeting   latest              1b7c061dad60        9 hours ago         490MB

. Run the container:

	docker container run -d -p 8080:8080 arungupta/greeting:jre
