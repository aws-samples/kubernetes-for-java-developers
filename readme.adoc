...
. Or, install without the Helm CLI:
...
Note: use the correct service name; it is `svc/myapp-greeting` if created with Helm,
or it is `svc/greeting` if created with kubectl.
